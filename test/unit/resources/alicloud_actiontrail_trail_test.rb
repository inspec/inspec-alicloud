...
class AliCloudActionTrailTrailConstructorTest < Minitest::Test
  def setup
    ...
    AliCloudActionTrailTrail.any_instance.stubs(:fetch_db_info).returns({ 'DBInstanceMemory' => 1024,
    ...
    AliCloudActionTrailTrail.any_instance.stubs(:fetch_vpc_info).returns({ 'VpcId' => 'vpc-1234',
                                                                              'IsDefault' => false })
  end
  ...
  def test_accepts_string_argument
    rds = AliCloudActionTrailTrail.new('not-there')
    assert_equal 'not-there', rds.instance_id
  end
  
  def test_accepts_key_value_argument_and_resource_works
    rds = AliCloudActionTrailTrail.new(db_instance_id: 'rm-inst4nc3')
  ...
  def test_accepts_instance_id_and_region
    rds = AliCloudActionTrailTrail.new(db_instance_id: 'rn-inst4nc3', region: 'eu-west-1')
